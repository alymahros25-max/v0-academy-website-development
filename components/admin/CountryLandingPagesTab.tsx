...
function AreaRecordEditor({ resource, record, onSave }: { resource: Resource; record: AreaRecord; onSave: (resource: Resource, id: number, changes: Record<string, unknown>) => Promise<void> }) {
        ...
        : resource === "packages"
          ...
          : resource === "content"
            ...
            : resource === "links"
              ...
              : resource === "themes"
                ? { theme_name_ar: values.theme_name_ar, theme_name_en: values.theme_name_en, primary_color: values.primary_color, secondary_color: values.secondary_color, accent_color: values.accent_color, background_color: values.background_color, text_color: values.text_color }
                : resource === "cities"
  ...
  return <div className="mt-3 grid gap-3 rounded-xl bg-muted/30 p-3">
    {resource === "themes" && <div className="grid gap-3 sm:grid-cols-2"><EditorInput label="اسم الهوية بالعربية" value={String(values.theme_name_ar ?? "")} onChange={(value) => setValue("theme_name_ar", value)} /><EditorInput label="اسم الهوية بالإنجليزية" value={String(values.theme_name_en ?? "")} onChange={(value) => setValue("theme_name_en", value)} />{(["primary_color", "secondary_color", "accent_color", "background_color", "text_color"] as const).map((key) => <EditorInput key={key} type="color" label={{ primary_color: "اللون الأساسي", secondary_color: "اللون الثانوي", accent_color: "لون التمييز", background_color: "لون الخلفية", text_color: "لون النص" }[key]} value={String(values[key] || "#000000")} onChange={(value) => setValue(key, value)} />)}</div>}
