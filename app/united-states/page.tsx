...
export default function UnitedStatesPage() {
  ...
  return <main dir="rtl" className="min-h-screen overflow-hidden bg-background">
    ...
    <section className="saudi-hero relative islamic-pattern text-center"><div className="saudi-flag-badge" aria-label="خدمة للعائلات العربية في أمريكا"><span aria-hidden="true">🇺🇸</span></div><div className="mx-auto max-w-4xl px-5 py-20 sm:px-8 lg:py-28"><p className="saudi-eyebrow justify-center"><Sparkles size={16} /> Online Quran Classes for Arabic-Speaking Families in the USA</p><h1 className="mt-6 text-balance text-4xl font-bold leading-tight text-foreground sm:text-6xl">تحفيظ القرآن وتعليم العربية أونلاين للعائلات العربية في أمريكا</h1><p className="mx-auto mt-6 max-w-2xl text-pretty text-lg leading-8 text-muted-foreground sm:text-xl">حصص فردية مباشرة للأطفال والشباب والنساء والرجال، مع معلمين ومعلمات، وموعد يناسب توقيتك داخل الولايات المتحدة.</p><div className="mx-auto mt-5 flex max-w-3xl flex-wrap justify-center gap-2 text-sm font-bold"><span className="rounded-full bg-secondary px-4 py-2 text-secondary-foreground">أول حصة تجريبية مجانية</span><span className="rounded-full bg-secondary px-4 py-2 text-secondary-foreground">خصم خاص للإخوة</span><span className="rounded-full bg-secondary px-4 py-2 text-secondary-foreground">مواعيد مرنة</span></div><div className="mt-8 flex flex-wrap justify-center gap-3"><a className="saudi-primary-cta" href={trialUrl} target="_blank" rel="noreferrer"><MessageCircle size={19} /> احجز حصتك التجريبية المجانية</a><a className="saudi-secondary-cta" href="#plans">استعرض الباقات <ChevronLeft size={18} /></a></div></div></section>
