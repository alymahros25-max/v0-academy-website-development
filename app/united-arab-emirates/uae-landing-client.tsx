...
import { uaeLandingConfig, type UaeProgram, type UaeDuration } from "@/lib/uae-landing-config"

export default function UaeLandingClient() {
  const [program, setProgram] = useState<UaeProgram>("quran")
  const [duration, setDuration] = useState<UaeDuration | "all">("all")
  ...
  function updateFilters(nextProgram: UaeProgram, nextDuration: UaeDuration | "all") {
  ...
  return <div className={`saudi-plan-nav-wrap ${visible ? "saudi-reveal-visible" : ""}`} aria-label="التنقل بين برامج وباقات الصفحة">
    <div className="mx-auto flex max-w-6xl flex-col items-center gap-4 px-5 py-6 sm:px-8">
      ...
      <p className="text-center text-sm text-muted-foreground">{uaeLandingConfig.plans.filter((plan) => plan.program === program).length} باقات متاحة · حصة تجريبية مجانية عبر واتساب</p>
    </div>
  </div>
}
