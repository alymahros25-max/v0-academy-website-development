...
function PlanCard({ plan }: { plan: (typeof uaeLandingConfig.plans)[number] }) {
  return <article data-saudi-program={plan.program} data-saudi-duration={plan.duration} className={`saudi-plan-card ${plan.popular ? "saudi-plan-card-featured" : ""}`}>
    ...
    <a className="saudi-plan-cta mt-7" href={getUaeWhatsAppUrl(plan.name)} target="_blank" rel="noreferrer"><MessageCircle size={18} /> جرب حصة مجانا</a>
  </article>
}

const planInfoByProgram = {
  quran: [
    ["تحفيظ قرآن أونلاين بخطة تناسب الطالب", "نساعد الطلاب والطالبات من دبي وأبوظبي والشارقة والعين وبقية مدن الإمارات على حفظ القرآن ومراجعته من خلال حصص فردية مرنة تناسب العمر والمستوى."],
    ...
    ["حصة تجريبية قبل اختيار الباقة", "ابدأ بحصة تجريبية مجانية ليتعرف المعلم على مستوى الطالب وهدفه، ثم نساعدك في اختيار الباقة المناسبة بالدرهم الإماراتي."],
  ],
  arabic: [
    ["تأسيس اللغة العربية أونلاين للأطفال والكبار", "برنامج تأسيس العربية مناسب للطلاب والطالبات في الإمارات، ويبدأ من مستوى الطالب في القراءة والكتابة والنطق دون افتراض مستوى سابق."],
    ...
    ["برنامج مرن يناسب وقت الأسرة", "يمكن اختيار مدة الحصة وعدد الحصص الأسبوعية والتنسيق على موعد مناسب بتوقيت الإمارات، مع إمكانية بدء التجربة قبل الاشتراك الشهري."],
  ],
} as const
...
const faqGroupsByProgram = {
  quran: [
    ...
    [["هل الحصص أونلاين من داخل الإمارات؟", "نعم، الحصص أونلاين ومتاحة من دبي وأبوظبي والشارقة وعجمان والعين ورأس الخيمة والفجيرة وأم القيوين وجميع مدن دولة الإمارات."], ["كيف أختار عدد الحصص المناسب؟", "نساعدك في الاختيار بعد معرفة عمر الطالب ومستواه وهدفه والوقت المتاح له، ويمكن البدء بحصة تجريبية مجانية."]],
  ],
  arabic: [
    ...
    [["هل البرنامج مناسب للأطفال والكبار؟", "نعم، توجد متابعة للأطفال والشباب والفتيات والرجال والنساء، ويتم اختيار المحتوى حسب مستوى كل طالب."], ["هل يمكن تحديد وقت يناسب الأسرة؟", "نعم، ننسق موعد الحصة بتوقيت الإمارات وفق الأوقات المتاحة للطالب والأسرة."]],
    [["هل الأسعار شهرية؟", "نعم، الأسعار المعروضة بالدرهم الإماراتي شهرية، وتختلف حسب مدة الحصة وعدد الحصص الأسبوعية."], ["كيف أحجز الحصة التجريبية؟", "اختر الباقة المناسبة واضغط جرب حصة مجانا، ثم أرسل بيانات الطالب والوقت المناسب عبر واتساب."]],
  ],
} as const
...
function PlansSection({ program, title }: { program: UaeProgram; title: string }) {
  const plans = uaeLandingConfig.plans.filter((plan) => plan.program === program && plan.visible)
...
export default function UaeArabiaPage() {
