...
function PlanCard({ plan, contactUrl }: { plan: AreaDisplayPlan; contactUrl?: string }) {
  return <article data-saudi-program={plan.program} data-saudi-duration={plan.duration} className={`saudi-plan-card ${plan.popular ? "saudi-plan-card-featured" : ""}`}>
    ...
    <a className="saudi-plan-cta mt-7" href={contactUrl ?? getUaeWhatsAppUrl(plan.name)} target="_blank" rel="noreferrer"><MessageCircle size={18} /> جرب حصة مجانا</a>
  </article>
}
...
export default async function UaeArabiaPage() {
  ...
  return <main dir="rtl" style={getCountryThemeStyle(areaData.theme)} className="min-h-screen overflow-hidden bg-background">
    <header className="bg-primary text-primary-foreground"><div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-5 py-3 sm:px-8"><Link href="/" className="flex items-center gap-3"><Image src="/logo.png" alt="شعار أكاديمية الحافظ المتميز" width={44} height={44} className="size-11 rounded-lg bg-secondary object-contain" priority /><span className="font-bold">أكاديمية الحافظ المتميز</span></Link><div className="flex items-center gap-3"><span className="text-2xl" aria-label="الإمارات العربية المتحدة" title="الإمارات العربية المتحدة" role="img">🇦🇪</span><a className="saudi-secondary-cta bg-primary-foreground px-3 py-2 text-sm text-primary" href={areaTrialUrl} target="_blank" rel="noreferrer">جرب حصة مجانا</a></div></div></header>
